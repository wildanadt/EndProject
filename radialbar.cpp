...
void RadialBar::paint(QPainter *painter)
{
    ...
    startAngle = -90 - static_cast<int>(m_StartAngle);
    if(FullDial != m_DialType)
    {
        spanAngle = 0 - static_cast<int>(m_SpanAngle);
    }
    else
    {
        spanAngle = -360;
    ...
    painter->setPen(pen);
    painter->drawArc(rect.adjusted(offset, offset, -offset, -offset), startAngle * 16,static_cast<int>(valueAngle) * 16);
    painter->restore();
}
